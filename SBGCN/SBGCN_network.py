...
class FaceEdgeVertexGCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x_e = self.F2E(x_f, x_e, data.face_to_edge[[1, 0]])
        x_v = self.E2V(x_e, x_v, data.edge_to_vertex[[1, 0]])
        ...
        # Downward Pass
        x_f = self.E2F(x_e, x_f, data.edge_to_face)
        x_e = self.V2E(x_v, x_e, data.vertex_to_edge)
        
        # Flatten Topology Representations
        n_topos = x_f.size(0) + x_e.size(0) + x_v.size(0)
        n_feats = x_f.size(1)
